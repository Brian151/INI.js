...
function INI(name,notStrict){
	...
	//determine if existing section with name <secName> exists
	...
	//determine if existing key named <keyName> exists within section <secName>
	...
	this.addSection = function(name){
		if(name){
			var failed=false;
			if(this.strict){
				failed=this.findDuplicateSections(name);
			}
			if(!failed){
	...
	this.addKey =  function(secName,keyName,keyValue){
		for (var i=0; i < this.sections.length; i++){
			if (this.sections[i][0] == secName) {
				var failed = false;
				if (this.strict) {
					failed = this.findDuplicateKeys(secName,keyName);
				}
				if (!failed) {
	...
	this.reNameSection = function(secName,secName2) {
		var failed = false;
		if (this.strict){
			failed = this.findDuplicateSections(secName2);
		}
		if (!failed){
			for (var i=0; i < this.sections.length; i++){
				if (this.sections[i][0] == secName) {
	...
	this.reNameKey = function(secName,keyName,keyName2){
		for (var i=0; i < this.sections.length; i++) {
			...
			if (this.sections[i][0] == secName){ 
				if (this.strict){
					failed = this.findDuplicateKeys(secName,keyName2);
				}
				if (!failed){
					for (var i2=0; i2 < this.sections[i][1].length; i2++) {
						if (this.sections[i][1][i2] == keyName) {
